                ...
                <x-forms.form-group class="mb-2">
                    <x-slot name="label">
                        <div class="flex-row flex gap-6 items-center">
                            <div class="w-1/4">
                                <x-forms.select
                                    wire:model.defer="patient_request.emergency_contact.phones.{{ $key }}.type"
                                ...
                                @error("patient_request.emergency_contact.phones.{$key}.type")
                                <x-forms.error>
                                    {{ $message }}
                                </x-forms.error>
                                @enderror
                            ...
                            <div class="w-1/4">
                                <x-forms.input x-mask="+380999999999" class="default-input"
                                               wire:model="patient_request.emergency_contact.phones.{{ $key }}.number"
                                               type="text"
                                               placeholder="{{ __('+ 3(80)00 000 00 00 ') }}"
                                />

                                @error("patient_request.emergency_contact.phones.{$key}.number")
                                <x-forms.error>
                                    {{ $message }}
                                </x-forms.error>
                                @enderror
